...
def GenerateMotion(
    ...
    while True:
        ...
        if method == "llm":
            ...
            prediction = call_vlm(message=msg, img=current_image, chat_processor=chat_processor, model=model, task="final")
            print(f'{prediction}')
...
if __name__ == '__main__':
    ...
    # Method to be used, combining VLM and LLM or only VLM
    if args.method == "llm":
    ...
    for scene in scenes:
        ...
        for i in range(scene_length - TOTAL_LENGTH):
            ...
            prediction, speed_curvature_pred, scene_description, object_description, intent_description = GenerateMotion(current_image=[current_image], past_waypoints=past_ego_traj_world, past_velocities=past_ego_velocities, past_curvatures=past_ego_curvatures, past_intent=prev_intent, chat_processor=vlm_chat_processor, model=vlm, llm_pipe=llm_pipe, verbose=args.verbose, method=args.method)
            # print(prediction)
            ...
            if args.verbose: cv2.imwrite(f"testtimgg.jpg", img)
            # Compute ADE.
